...
@pytest.mark.asyncio
async def test_discover_cache_hit_still_requires_portfolio_access():
    import app.routers.portfolio as portfolio_module
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
        ...
        intruder_token = await _register_and_token(c, intruder_email, create_invite_token(intruder_email))
