...
@pytest.mark.unit
@pytest.mark.asyncio
async def test_discover_authorizes_before_cached_response_and_provider_lookup():
    import app.routers.portfolio as portfolio_module
    ...
    old_cross_user_key = f"{portfolio_id}:openai:gpt-4o-mini:{body.response_language}"
    portfolio_module._discover_cache[old_cross_user_key] = (
        [{"ticker": "LEAK", "tag": "Gap Fill", "sector": "Healthcare", "reason": "private context"}],
        time.time() + 60,
    )
