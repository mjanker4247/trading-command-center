...
async def test_discover_cached_results_are_authorized_per_user():
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
        ...
        intruder_invite = create_invite_token("discover-intruder@test.com")
        intruder_register = await c.post(
            "/auth/register",
            json={
                "email": "discover-intruder@test.com",
                "password": "pass1234",
                "name": "Intruder",
                "invite_token": intruder_invite,
            },
        )
        assert intruder_register.status_code == 200
