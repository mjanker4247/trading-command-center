...
@pytest.mark.unit
@pytest.mark.asyncio
async def test_discover_clears_in_flight_when_candidate_building_fails():
    import app.routers.portfolio as portfolio_module
    ...
    db = AsyncMock()
    db.execute.return_value = holdings_result
    body = portfolio_module.DiscoverRequest(llm_provider="openai", llm_model="gpt-4o-mini")
    cache_key = f"{user.id}:{portfolio_id}:openai:gpt-4o-mini:en-US"
    ...
    try:
        with (
            ...
            with pytest.raises(RuntimeError):
                await portfolio_module.discover_stocks(portfolio_id, body, db=db, user=user)
