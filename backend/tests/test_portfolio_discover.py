...
@pytest.mark.asyncio
async def test_discover_cached_result_requires_portfolio_owner():
    import app.routers.portfolio as portfolio_module
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
        owner_token = await _register_and_token(c, "discover-cache-owner@test.com")
        portfolio_id = await _create_portfolio_with_holding(c, owner_token)
        other_token = await _register_and_token(c, "discover-cache-other@test.com")
        body = {"llm_provider": "openai", "llm_model": "gpt-4o-mini"}
    ...
    assert other_response.json()["detail"] == "Portfolio not found"


@pytest.mark.asyncio
async def test_discover_cleans_in_flight_after_sector_gap_failure():
    import app.routers.portfolio as portfolio_module
    ...
    async with AsyncClient(
        ...
        token = await _register_and_token(c, "discover-inflight-cleanup@test.com")
        portfolio_id = await _create_portfolio_with_holding(c, token)
        body = {"llm_provider": "openai", "llm_model": "gpt-4o-mini"}
