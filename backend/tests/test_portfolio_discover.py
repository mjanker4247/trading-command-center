...
async def _seed_user_token(email: str) -> str:
    async with AsyncSessionLocal() as db:
        user = User(email=email, name="Other User", role=UserRole.member)
        db.add(user)
        await db.commit()
...
@pytest.mark.asyncio
async def test_discover_cached_response_still_requires_portfolio_owner():
    import app.routers.portfolio as portfolio_module
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
        ...
        other_token = await _seed_user_token("discover-cache-other@test.com")
...
@pytest.mark.asyncio
async def test_discover_cleans_in_flight_marker_when_candidate_fetch_fails():
    import app.routers.portfolio as portfolio_module
