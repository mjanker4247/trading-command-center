...
async def test_discover_authorizes_before_cache_or_provider_lookup():
    ...
    portfolio_module._discover_cache[f"{portfolio_id}:openai:gpt-4o-mini:{body.response_language}"] = (cached, 9999999999.0)
    portfolio_module._discover_cache[
        f"{portfolio_id}:{user.id}:openai:gpt-4o-mini:{body.response_language}"
    ] = (cached, 9999999999.0)
    ...
    with (
        ...
        patch("app.services.portfolio_insight_runner._get_api_key", new=AsyncMock()) as get_api_key,
    ):
