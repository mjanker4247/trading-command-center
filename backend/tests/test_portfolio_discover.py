...
async def _register_invited_and_token(client: AsyncClient, email: str) -> str:
    invite_token = create_invite_token(email)
    r = await client.post(
        "/auth/register",
        json={"email": email, "password": "pass1234", "name": "Test", "invite_token": invite_token},
    )
...
@pytest.mark.asyncio
async def test_discover_cache_hit_still_requires_portfolio_owner():
    import app.routers.portfolio as portfolio_module
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
        ...
        intruder_token = await _register_invited_and_token(c, "discover-intruder@test.com")
...
@pytest.mark.asyncio
async def test_discover_clears_in_flight_after_candidate_pipeline_exception():
    import app.routers.portfolio as portfolio_module
    ...
    async with AsyncClient(
        ...
        token = await _register_and_token(c, "discover-pipeline-error@test.com")
        portfolio_id = await _create_portfolio_with_holding(c, token)
