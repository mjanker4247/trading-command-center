...
async def test_discover_authorizes_before_serving_cached_recommendations():
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
        ...
        async with AsyncSessionLocal() as db:
            attacker = User(
                email="discover-attacker@test.com",
            ...
            attacker_token = create_access_token(str(attacker.id), attacker.role.value)
    ...
    assert "recommendations" not in attacker_response.json()
