...
async def test_discover_authorizes_before_cached_return_unit():
    ...
    # Old cache keys lacked user identity; an unauthorized caller must not receive
    # this entry before ownership is checked.
    portfolio_module._discover_cache[
        f"{portfolio_id}:openai:gpt-4o-mini:en-US"
    ] = ([{"ticker": "LEAK", "tag": "Gap Fill", "sector": "Technology", "reason": "private"}], time.time() + 60)

    with (
        patch("app.routers.portfolio._get_latest_snapshot", new=AsyncMock(side_effect=HTTPException(status_code=404))),
...
@pytest.mark.unit
@pytest.mark.asyncio
async def test_discover_clears_in_flight_when_candidate_fetch_fails_unit():
    import app.routers.portfolio as portfolio_module
    ...
    with (
        patch("app.routers.portfolio._get_latest_snapshot", new=AsyncMock(return_value=snapshot)),
        patch("app.services.portfolio_insight_runner._get_api_key", new=AsyncMock(return_value="sk-test")),
...
@pytest.mark.asyncio
async def test_discover_authorizes_before_serving_cached_result():
    import app.routers.portfolio as portfolio_module
...
async def test_discover_clears_in_flight_when_candidate_fetch_fails():
    ...
    async with AsyncClient(
        ...
        token = await _register_and_token(c, "discover-failure@test.com")
        portfolio_id = await _create_portfolio_with_holding(c, token)
