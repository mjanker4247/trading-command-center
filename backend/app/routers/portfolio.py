...
async def discover_stocks(
    ...
    # Authorize before touching provider/key/cache state; cached recommendations
    # are derived from portfolio holdings and must not be served cross-user.
    try:
        snap = await _get_latest_snapshot(portfolio_id, user.id, db)
    except HTTPException as exc:
        if exc.detail == "No snapshot found — upload a CSV first":
    ...
    cache_key = f"{user.id}:{portfolio_id}:{llm_provider}:{llm_model}:{body.response_language}"
    now = time.time()
    ...
    elif cache_key in _discover_in_flight:
        raise HTTPException(status_code=409, detail="Discovery already in progress.")
...
{response_language_instruction(body.response_language, json_values=True)}"""
        ...
        if recommendations:
            _discover_cache[cache_key] = (recommendations, time.time() + _DISCOVER_TTL)
        return {
