...
async def discover_stocks(
    ...
    # Validate ownership before any cache/in-flight fast path. Discover output is
    # derived from private holdings, so cached responses must stay owner-scoped.
    snap = await _get_latest_snapshot(portfolio_id, user.id, db)
    ...
    # The in-flight marker is a plain set, so never add the same key twice.
