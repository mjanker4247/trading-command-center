...
async def discover_stocks(
    ...
    if cache_key in _discover_in_flight:
        cached_entry = None if body.force_refresh else _discover_cache.get(cache_key)
        if cached_entry:
            return {"recommendations": cached_entry[0], "cached": True, "empty_reason": None, "candidate_count": None}
        raise HTTPException(
            status_code=409,
            detail="Stock discovery is already running for this portfolio. Try again shortly.",
        )
