...
async def discover_stocks(
    ...
    # Avoid duplicate work for the same user/portfolio/config. Without a cached
    # result, report contention instead of a false "no candidates" outcome.
    if cache_key in _discover_in_flight:
        ...
        raise HTTPException(
            status_code=409,
            detail="Stock discovery is already running. Try again shortly.",
        )
