...
async def discover_stocks(
    ...
    # Authorize before provider/key lookups or process-local cache reads.
    try:
        snap = await _get_latest_snapshot(portfolio_id, user.id, db)
    except HTTPException:
    ...
    if cache_key in _discover_in_flight:
        if not body.force_refresh:
            ...
            if cached_entry:
                ...
                if now < expiry:
                    return {"recommendations": cached, "cached": True, "empty_reason": None, "candidate_count": None}
        raise HTTPException(status_code=409, detail="Stock discovery is already running for this portfolio.")
