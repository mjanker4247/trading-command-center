...
async def discover_stocks(
    ...
    # Authorize before shared provider/cache/in-flight work; cached recommendations
    # include portfolio-derived context and must never be served cross-user.
    try:
        snap = await _get_latest_snapshot(portfolio_id, user.id, db)
    except HTTPException:
    ...
    cache_key = f"{portfolio_id}:{user.id}:{llm_provider}:{llm_model}:{body.response_language}"
    now = time.time()
    ...
    if cache_key in _discover_in_flight:
        ...
        if cached_entry and not body.force_refresh:
