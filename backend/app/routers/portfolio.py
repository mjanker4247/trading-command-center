...
async def discover_stocks(
    ...
    # Force-refresh callers should wait for the active generation rather than start
    # duplicate work because the in-flight marker is not reference counted.
    if cache_key in _discover_in_flight:
        cached_entry = _discover_cache.get(cache_key)
        if not body.force_refresh and cached_entry:
            cached, expiry = cached_entry
            if now < expiry:
                return {"recommendations": cached, "cached": True, "empty_reason": None, "candidate_count": None}
        raise HTTPException(status_code=409, detail="Stock discovery is already in progress.")
