...
async def discover_stocks(
    ...
    now = time.time()
    if cache_key in _discover_cache and not body.force_refresh:
        cached, expiry = _discover_cache[cache_key]
        if now < expiry:
            return {"recommendations": cached, "cached": True, "empty_reason": None, "candidate_count": None}

    # Return last fresh result if available; otherwise report that work is active.
    if cache_key in _discover_in_flight:
        ...
        raise HTTPException(status_code=409, detail="Stock discovery already in progress.")
