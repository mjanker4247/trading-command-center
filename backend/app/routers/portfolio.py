...
async def discover_stocks(
    ...
    # Authorize before cache/provider work; cached recommendations include portfolio context.
    try:
        snap = await _get_latest_snapshot(portfolio_id, user.id, db)
    except HTTPException:
    ...
    if cache_key in _discover_in_flight:
        ...
        raise HTTPException(status_code=409, detail="Discovery is already running for this portfolio.")
