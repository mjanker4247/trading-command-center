...
function isCurrentPrefetch(generation: number, queryClient: QueryClient): boolean {
  if (generation === prefetchGeneration) return true;
  queryClient.clear();
  return false;
}
...
async function runPrefetch(queryClient: QueryClient, generation: number): Promise<void> {
  ...
  if (!isCurrentPrefetch(generation, queryClient)) return;
  ...
  await Promise.all(prefetches);
  isCurrentPrefetch(generation, queryClient);
}
